...
import sen.manaita_plus_legacy.common.entity.ManaitaPlusLightningPlusBolt;

import java.awt.*;

import static sen.manaita_plus_legacy.common.util.ManaitaPlusColor.milliTime;
import static sen.manaita_plus_legacy.common.util.ManaitaPlusColor.rangeRemap;

@OnlyIn(Dist.CLIENT)
public class ManaitaPlusLightningPlusBoltRenderer extends EntityRenderer<ManaitaPlusLightningPlusBolt> {
    public ManaitaPlusLightningPlusBoltRenderer(EntityRendererProvider.Context p_174286_) {
        super(p_174286_);
    }

    public void render(ManaitaPlusLightningPlusBolt p_115266_, float p_115267_, float p_115268_, PoseStack p_115269_, MultiBufferSource p_115270_, int p_115271_) {
        ...
        float huehuehue = (milliTime() / 700f) + (randomsource.nextInt(1000) / 1000.0F) % 1;
        float huehuehueStep;
        for(int j = 0; j < 4; ++j) {
...
//                    float[] colors = Color.RGBtoHSB(r,g,b, null);
                    huehuehueStep = (float) rangeRemap(Math.sin(milliTime() / 1200f) % 6.28318, -0.9, 2.5, 0.025, 0.15);
                    Color color = Color.getHSBColor(huehuehue,.8f,0.5f);
    ...
    public ResourceLocation getTextureLocation(ManaitaPlusLightningPlusBolt p_115264_) {
