...
public class ManaitaPlusHoeItem extends DiggerItem implements IManaitaPlusKey, IManaitaPlusDestroy {
    public ManaitaPlusHoeItem() {
        super(ManaitaPlusItemTier.MAX, ManaitaPlusItemTier.MAX, new ManaitaPlusItemTier(), BlockTags.MINEABLE_WITH_HOE, new Properties().fireResistant());
    }
    ...
    public void onManaitaKeyPressOnClient(ItemStack itemStack, Player player) {
        boolean doubling = !getDoubling(itemStack);
        setDoubling(itemStack, doubling);
        ManaitaPlusUtils.chat(Component.literal(ManaitaPlusText.manaita_mode.formatting(String.format("[%s] %s: %s", I18n.get("item.manaita_plus_legacy.manaita_hoe"),I18n.get("mode.doubling"), (doubling ? I18n.get("info.on") : I18n.get("info.off"))))));
    }
    ...
    public void setRange(ItemStack itemStack,int range) {
        ...
        ManaitaPlusUtils.chat(Component.literal(ManaitaPlusText.manaita_mode.formatting("[" + I18n.get("item.manaita_plus_legacy.manaita_hoe") + "] " +I18n.get("mode.range.name") + ": " + range + "x" + range + "x" + range)));
    }
